      ...
      <div className="p-8 max-w-7xl mx-auto space-y-8">
        ...
        <div className="glass-card border-yellow-500/30 p-8 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-6 relative z-10">
            <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-yellow-400 to-orange-600 flex items-center justify-center text-3xl font-black text-white shadow-[0_0_30px_rgba(234,179,8,0.4)]">
              {mockUser.level}
            </div>
            <div className="flex-1">
              <div className="text-sm font-bold text-yellow-500 uppercase tracking-widest mb-1">Current Rank</div>
              <h2 className="text-3xl font-bold text-white mb-4">{mockUser.levelName}</h2>
              
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-white">{mockUser.xp.toLocaleString()} XP</span>
            ...
            {achievements.map((ach, i) => (
              <motion.div
                ...
                className={`glass-card p-6 flex flex-col items-center justify-center text-center gap-3 border ${
                  ach.unlocked ? 'border-primary/30 hover:border-primary/60 hover:-translate-y-1 transition-all' : 'border-white/5 opacity-50 grayscale'
                }`}
              >
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${ach.unlocked ? 'bg-primary/20 text-primary glow-purple' : 'bg-white/10 text-gray-500'}`}>
                  {ach.unlocked ? <Star size={24} /> : <Target size={24} />}
                </div>
                <div>
                  <div className="font-bold text-sm text-white leading-tight">{ach.name}</div>
