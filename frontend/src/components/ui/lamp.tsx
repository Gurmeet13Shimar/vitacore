  ...
  return (
    ...
    >
      ...
      <div className="relative flex w-full flex-1 scale-y-125 items-center justify-center isolate z-0">
        ...
        >
          {/* Mask: clip bottom edge */}
          <div className="absolute w-[100%] left-0 bg-slate-950 h-40 bottom-0 z-20 [mask-image:linear-gradient(to_top,white,transparent)]" />
          {/* Mask: clip right edge */}
          <div className="absolute w-40 h-[100%] left-0 bg-slate-950 bottom-0 z-20 [mask-image:linear-gradient(to_right,white,transparent)]" />
        </motion.div>

        {/* Right conic gradient beam (mirror) */}
        <motion.div
          initial={{ opacity: 0.5, width: "15rem" }}
          whileInView={{ opacity: 1, width: "30rem" }}
          transition={{ delay: 0.3, duration: 0.8, ease: "easeInOut" }}
        ...
        <motion.div
          ...
          transition={{ delay: 0.3, duration: 0.8, ease: "easeInOut" }}
          viewport={{ once: true }}
          className="absolute inset-auto z-30 h-0.5 -translate-y-[7rem] bg-cyan-400"
        />

        {/* ── Wide blur ellipse glow ── */}
        ...
        <motion.div
          ...
          transition={{ delay: 0.3, duration: 0.8, ease: "easeInOut" }}
          viewport={{ once: true }}
          className="absolute inset-auto z-50 h-36 -translate-y-[6rem] rounded-full bg-cyan-400 blur-2xl"
        />
      ...
      {/* ── Children rendered in front of lamp ── */}
      <div className="relative z-50 flex flex-col items-center px-5 -translate-y-80 w-full">
        {children}
      </div>
    </div>
  );
